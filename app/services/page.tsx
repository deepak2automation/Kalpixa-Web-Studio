...
const groupVisuals = [
  '/studio/service-platforms.webp',
  '/studio/service-search.webp',
  '/studio/service-campaigns.webp',
  '/studio/service-measurement.webp',
  '/studio/service-commerce.webp',
  '/studio/service-trust.webp',
] as const;

export default function ServicesPage() {
  return <main id="main-content" className="original-services-page">
    <PageHero eyebrow="Capabilities · 29 specialties" title="Services built as connected growth systems." lead="Choose a focused engagement or combine the capabilities needed to plan, build, launch, measure and improve your digital presence." image="services"/>
    <nav className="services-navigator shell" aria-label="Service capability groups"><div className="services-navigator-intro"><p className="eyebrow">Capability directory</p><h2>Move directly to the expertise you need.</h2></div><div className="services-navigator-grid">{capabilityGroups.map((group) => <a href={`#${group.slug}`} key={group.slug}><span>{group.number}</span><strong>{group.title}</strong><small>{group.capabilities.length} capabilities</small></a>)}</div></nav>
    <section className="service-catalog shell" aria-labelledby="delivery-services-heading">
      <div className="catalog-intro"><div><p className="eyebrow">Core delivery services</p><h2 id="delivery-services-heading">Practical starting points.</h2></div><p>Starting prices are directional, not fixed offers. Final scope, dependencies, delivery plan and commercial terms are confirmed in writing before work begins.</p></div>
      {services.map((service) => <article className="catalog-card" id={service.slug} key={service.slug}><div className="catalog-card-top"><span>Service {service.number}</span><strong>{service.price}</strong></div><h3>{service.title}</h3><p>{service.short}</p><ul>{service.features.map((feature) => <li key={feature}>{feature}</li>)}</ul><a className="text-link" href="/contact/">Discuss this service →</a></article>)}
    </section>
    <section className="capability-program" aria-labelledby="specialties-heading">
      ...
      {capabilityGroups.map((group, groupIndex) => <section className="capability-group shell" id={group.slug} key={group.slug} aria-labelledby={`${group.slug}-heading`}><header><span>{group.number}</span><div><h2 id={`${group.slug}-heading`}>{group.title}</h2><p>{group.summary}</p></div></header><figure className="capability-editorial"><img src={groupVisuals[groupIndex]} alt="" aria-hidden="true" loading="lazy" decoding="async"/></figure><div className="capability-list">{group.capabilities.map((capability, index) => <article key={capability.name}><span>{String(index + 1).padStart(2, '0')}</span><div><h3>{capability.name}</h3>{capability.alias && <small>Also searched as: {capability.alias}</small>}<p>{capability.description}</p></div></article>)}</div><footer><strong>Intended outcome</strong><p>{group.outcome}</p></footer></section>)}
    </section>

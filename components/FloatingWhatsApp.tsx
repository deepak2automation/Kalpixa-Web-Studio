...
export function FloatingWhatsApp() {
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => setPosition(fit(initial)));

    const resize = () => setPosition((value) => fit(value ?? defaultPosition()));
  ...
  function pointerMove(event: ReactPointerEvent<HTMLAnchorElement>) {
    ...
    setPosition(fit({ x: start.current.x + deltaX, y: start.current.y + deltaY }));
  }
